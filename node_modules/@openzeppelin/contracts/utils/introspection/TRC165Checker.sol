...
library TRC165Checker {
    ...
    function supportsInterface(address account, bytes4 interfaceId) internal view returns (bool) {
        ...
        return supportsTRC165(account) && _supportsTRC165Interface(account, interfaceId);
    }
        ...
        returns (bool[] memory)
    {
        ...
        if (supportsTRC165(account)) {
            ...
            for (uint256 i = 0; i < interfaceIds.length; i++) {
                interfaceIdsSupported[i] = _supportsTRC165Interface(account, interfaceIds[i]);
            }
        }
    ...
    function _supportsTRC165Interface(address account, bytes4 interfaceId) private view returns (bool) {
        bytes memory encodedParams = abi.encodeWithSelector(ITRC165.supportsInterface.selector, interfaceId);
