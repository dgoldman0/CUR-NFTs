...
/**
 * @dev Implementation of the TRC3156 Flash loans extension, as defined in
...
abstract contract TRC20FlashMint is TRC20, ITRC3156FlashLender {
    bytes32 private constant _RETURN_VALUE = keccak256("TRC3156FlashBorrower.onFlashLoan");
    ...
    function maxFlashLoan(address token) public view override returns (uint256) {
        return token == address(this) ? type(uint256).max - TRC20.totalSupply() : 0;
    }
    ...
    function flashFee(address token, uint256 amount) public view virtual override returns (uint256) {
        require(token == address(this), "TRC20FlashMint: wrong token");
    ...
    function flashLoan(
        ...
        require(currentAllowance >= amount + fee, "TRC20FlashMint: allowance does not allow refund");
